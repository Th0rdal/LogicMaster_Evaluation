    ...
    try:
        if os.path.exists(modelPath):
            with open("/app/model/ai.pkl", "rb") as f:
                ai = pickle.load(f)
            model = DDPG.load(modelPath, env=ai)
            ai.configModelAndAI(model, modelPath)
            logger.info("Loaded model successfully!")
        else:
            logger.info("No model found. Creating new model!")
            ai = BaseEnvironment(spaces.Box(low=-10, high=10, dtype=np.float64))
            model = DDPG("MlpPolicy", ai, verbose=1)
            ai.configModelAndAI(model, modelPath)
